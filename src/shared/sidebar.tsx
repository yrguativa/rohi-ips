...
export default function Sidebar() {
    ...
    return (
        <aside className={"absolute left-0 top-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black duration-300 ease-linear  lg:static lg:translate-x-0 dark:bg-boxdark"
            + (sidebarToggle ? 'translate-x-0' : '-translate-x-full')}
            onClick={() => setSidebarToggle(false)}>
            <div className="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5">
                <div className="bg-white ml-3 mr-3 pl-4 pr-4 pt-2 pb-2 rounded-md">
                    <NavLink to="/" >
                ...
                <button
                    className="block md:hidden" onClick={() => setSidebarToggle((nowState: boolean) => !nowState)}>
            ...
            <div className="no-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
                <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6" x-data="{selected: $persist('Dashboard')}">
                    <div>
                        <h3 className="mb-4 ml-4 text-sm font-medium text-bodydark2">MENU</h3>

                        <ul className="mb-6 flex flex-col gap-1.5">
                            {
                                (!roles?.isAdmin && !roles?.isRadioUser) && (
                                    <li>
                                        <NavLink className={({ isActive }) => 'group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 dark:bg-boxdark ' +
